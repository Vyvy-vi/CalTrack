...
class AlarmManagerBroadcast : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        pendingIntent = PendingIntent.getActivity(
            context, 0, new_intent, PendingIntent.FLAG_IMMUTABLE
        )
        notificationManager = context.getSystemService(NOTIFICATION_SERVICE) as NotificationManager
        supportAction =
            Notification.Action.Builder(R.drawable.ic_alerts, "Support", pendingIntent).build()
    ...
    private fun myNotificationChannel(context: Context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            builder = Notification.Builder(context, channelId).setSmallIcon(R.drawable.ic_water)
                .setContentTitle(title).setContentText(description).setLargeIcon(
                    BitmapFactory.decodeResource(
                        context.resources, R.drawable.ic_waterlog
                    )
                ).setContentIntent(pendingIntent).addAction(supportAction).setAutoCancel(true)

        } else {
            builder = Notification.Builder(context).setSmallIcon(R.drawable.ic_water)
                .setContentTitle(title).setContentText(description).setLargeIcon(
                    BitmapFactory.decodeResource(
                        context.resources, R.drawable.ic_waterlog
                    )
